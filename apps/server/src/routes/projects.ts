...
router.get('/:id/export.srt', async (req, res) => {
  ...
  const asciiTitle = rawTitle.replace(/[^a-zA-Z0-9_-]+/g, '_') || 'captions';
  const encodedTitle = encodeURIComponent(rawTitle || 'captions');
  res.setHeader('Content-Type', 'application/x-subrip; charset=utf-8');
  res.setHeader('Content-Disposition', `attachment; filename="${asciiTitle}.srt"; filename*=UTF-8''${encodedTitle}.srt`);
  res.send(srt);
});
