...
export default function App() {
  ...
  const exportSrt = async () => {
    ...
    try {
      ...
      const saved = await saveDraft(true, 'manual-save', true);
      if (hadUnsavedEdits && !saved) return;
      const rawTitle = (project.title || 'captions').trim();
      const safeFilename = `${rawTitle.replace(/[/\\?%*:|"<>]/g, '_') || 'captions'}.srt`;
      const res = await fetch(`/api/projects/${project.id}/export.srt`);
  ...
  if (!project) return <main>
    <div className="home-tools">
      {activeJobs.length > 0 && <button onClick={() => setShowJobs(true)}><ListTodo size={16}/>{t('activity', 'Activity')}<b>{activeJobs.length}</b></button>}
      {pendingCorrections > 0 && <button onClick={() => setShowCorrections(true)}><BookOpenCheck size={16}/>{t('correctionInbox', 'Corrections')}<b>{pendingCorrections}</b></button>}
      <button onClick={toggleLang} title={t('toggleLanguage', 'Switch language (Khmer / English)')} aria-label="Toggle language"><Languages size={16}/><span>{lang === 'km' ? 'ភាសាខ្មែរ' : 'English'}</span></button>
      <button onClick={toggleTheme} title={theme === 'dark' ? t('themeLight', 'Switch to light mode') : t('themeDark', 'Switch to dark mode')} aria-label="Toggle theme">{theme === 'dark' ? <Sun size={16}/> : <Moon size={16}/>}<span>{theme === 'dark' ? 'Light' : 'Dark'}</span></button>
      <button className={llmSettings?.configured ? '' : 'setup-needed'} title="Connection, profile, and system check" onClick={() => openSettings('ai')}><Settings2 size={16}/>{t('settings', 'Settings')}<span className={`connection-dot ${llmSettings?.configured ? 'ready' : 'missing'}`}/></button>
    </div>
  ...
  return <main className="workspace">
    <header>
      <button className="back" aria-label={t('back', 'Back to projects')} title={t('back', 'Back to projects')} onClick={() => setProject(null)}><ChevronLeft size={18}/></button>
      <div className="workspace-identity"><div className="project-title"><strong>{project.title}</strong><span>{project.media.originalName} · {dirty ? autosaveState === 'saving' ? 'autosaving…' : 'autosave pending' : 'saved'}{project.transcriptNeedsSync ? ' · transcript regrouping needs refresh' : ''}</span></div></div>
      ...
      <div className="header-actions">
        <button className={reviewMode ? 'selected-tool' : ''} title="Review captions worth checking" aria-pressed={reviewMode} onClick={() => chooseWorkspaceTool('review')}><ShieldCheck size={16}/><span>{t('review', 'Review')}</span>{issues.length > 0 && <b className="tool-badge">{issues.length}</b>}</button>
        {/* WorkspaceToolsMenu hidden */}
        <button className="icon-btn lang-toggle-btn" onClick={toggleLang} title={t('toggleLanguage', 'Switch language (Khmer / English)')} aria-label="Toggle language" style={{ fontWeight: 700, fontSize: '11px' }}>{lang === 'km' ? 'ខ្មែរ' : 'EN'}</button>
        <button className="icon-btn theme-toggle-btn" onClick={toggleTheme} title={theme === 'dark' ? t('themeLight', 'Switch to light mode') : t('themeDark', 'Switch to dark mode')} aria-label="Toggle theme">{theme === 'dark' ? <Sun size={16}/> : <Moon size={16}/>}</button>
        <button className="save-action" disabled={!dirty || !!busy} title={dirty ? 'Save changes' : 'All changes saved'} onClick={() => void saveDraft(false, 'manual-save', true)}><Save size={16}/><span>{dirty ? t('save', 'Save') : t('saved', 'Saved')}</span></button>
        <button className={`primary ${draft.length ? '' : 'disabled'} ${workspaceTool === 'export' ? 'selected-tool' : ''}`} title="Export SRT or a finished captioned video" aria-pressed={workspaceTool === 'export'} disabled={!draft.length || !!busy} onClick={() => chooseWorkspaceTool('export')}><Download size={16}/><span>{t('export', 'Export')}</span></button>
      </div>
    </header>
