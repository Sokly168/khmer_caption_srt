  ...
  return <div className="caption-panel">
    <div className="panel-head">
      <div className="panel-head-copy">
        <strong>{reviewMode ? t('reviewQueue', 'Review queue') : t('timelineCaptions', 'Timeline captions')}</strong>
        <span>{visible.length} shown · {captions.length} total · {followPlayback ? 'following playback' : 'follow paused — browse freely'}</span>
      </div>
      <div className="panel-tools">
        <button className="panel-tool" title="Jump to current caption" onClick={jumpToPlayhead} disabled={!visible.length}><LocateFixed size={15}/><span>{t('current', 'Current')}</span></button>
        <button className={`panel-tool ${followPlayback ? 'following' : ''}`} title={followPlayback ? 'Pause follow' : 'Follow playback'} onClick={toggleFollow} disabled={!visible.length}>{followPlayback ? <Pause size={14}/> : <Play size={14}/>}<span>{followPlayback ? 'Following' : t('follow', 'Follow')}</span></button>
        <button className="icon-btn" title={t('addCaption', 'Add caption')} onClick={add}><Plus size={17}/></button>
      </div>
    </div>
    <div
      ref={list}
