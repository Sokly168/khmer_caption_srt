...
export function WaveformEditor({
  ...
  useEffect(() => {
    ...
    for (const caption of captions) {
      ...
      ctx.fillStyle = isSelected ? 'rgba(0,229,255,.15)' : caption.approved ? 'rgba(0,229,255,.08)' : 'rgba(126,138,154,.055)';
      ctx.fillRect(x1, 126, Math.max(2, x2 - x1), 30);
      ctx.strokeStyle = caption.timingLocked ? '#e7b95d' : isSelected ? '#00e5ff' : '#48505b';
    ...
    for (const token of tokens) {
      ...
      ctx.strokeStyle = token.timingSource === 'interpolated' ? '#d56565' : '#0891b2';
      ctx.beginPath(); ctx.moveTo(x, 112); ctx.lineTo(x, 124); ctx.stroke();
      if (showWords) {
        ctx.fillStyle = token.timingSource === 'interpolated' ? '#d78d8d' : '#67e8f9';
